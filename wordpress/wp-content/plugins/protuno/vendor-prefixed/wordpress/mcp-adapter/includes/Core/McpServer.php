...
use Protuno\Deps\WP\MCP\Infrastructure\ErrorHandling\NullMcpErrorHandler;
use Protuno\Deps\WP\MCP\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface;
use Protuno\Deps\WP\MCP\Infrastructure\Observability\NullMcpObservabilityHandler;
use Protuno\Deps\WP\MCP\Transport\Infrastructure\McpTransportContext;
...
class McpServer {
	 ...
	 *
	 * @var \Protuno\Deps\WP\MCP\Core\McpComponentRegistry
	 */
	 ...
	 *
	 * @var \Protuno\Deps\WP\MCP\Core\McpTransportFactory
	 */
	 ...
	 * @param array<class-string<\Protuno\Deps\WP\MCP\Transport\Contracts\McpTransportInterface>> $mcp_transports Array of MCP transport class names to initialize (e.g., [McpRestTransport::class]).
	 * @param class-string<\Protuno\Deps\WP\MCP\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface>|null $error_handler Error handler class to use (e.g., NullMcpErrorHandler::class). Must implement McpErrorHandlerInterface. If null, NullMcpErrorHandler will be used.
	 * @param class-string<\Protuno\Deps\WP\MCP\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface>|null $observability_handler Observability handler class to use (e.g., NullMcpObservabilityHandler::class). Must implement McpObservabilityHandlerInterface. If null, NullMcpObservabilityHandler will be used.
	...
	public function __construct(
		 ...
		 * @param \Protuno\Deps\WP\MCP\Core\McpServer $server    The McpServer instance being constructed.
		 */
	...
	private function setup_handlers( ?string $error_handler, ?string $observability_handler ): void {
		...
		if ( $error_handler && class_exists( $error_handler ) ) {
			/** @var \Protuno\Deps\WP\MCP\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface $handler */
		...
		if ( $observability_handler && class_exists( $observability_handler ) ) {
			/** @var \Protuno\Deps\WP\MCP\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface $handler */
	 ...
	 * @param array<class-string<\Protuno\Deps\WP\MCP\Transport\Contracts\McpTransportInterface>> $mcp_transports Transport classes to initialize.
	 *
	 * @throws \Exception
	 */
	 ...
	 *
	 * @return \Protuno\Deps\WP\MCP\Infrastructure\Observability\Contracts\McpObservabilityHandlerInterface
	 */
	 ...
	 * @since 0.5.0
	 *
	 * @return \Protuno\Deps\WP\MCP\Infrastructure\ErrorHandling\Contracts\McpErrorHandlerInterface
	 */
	 ...
	 *
	 * @return \Protuno\Deps\WP\McpSchema\Server\Prompts\DTO\Prompt|null
	 */
	 ...
	 *
	 * @return \Protuno\Deps\WP\MCP\Transport\Infrastructure\McpTransportContext
	 */
