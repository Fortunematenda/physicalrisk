...
if ( ! class_exists( 'Protuno_Globals' ) ) {
	...
	class Protuno_Globals {
        ...
        private static function save_all_kit_settings( $document_settings ) {
            ...
            foreach($all_users as $user){
                ...
                if( $autosave ){
                    ...
                    Protuno_Globals::elementor_refresh_css_and_clear_cache( $autosave->get_id() );
                }
            }

            // Clear Cache & reset CSS
            Protuno_Globals::elementor_refresh_css_and_clear_cache( $kit->get_id() );
        }
        ...
        private static function ensure_default_container_widths($all_settings) {
            ...
            if ($needs_update) {
                Protuno_Globals::save_all_kit_settings($all_settings);
            }
        ...
        public static function get_elementor_container_breakpoints_width(){
            ...
            $all_settings = self::ensure_default_container_widths(Protuno_Globals::get_all_kit_settings());
        ...
        public static function set_container_breakpoints_width($new_container_width){
            ...
            $document_settings = Protuno_Globals::get_all_kit_settings();
            ...
            // Save the settings
            Protuno_Globals::save_all_kit_settings($document_settings);
        ...
        public static function delete_global_typography( $id ) {
            ...
            // Save
            return Protuno_Globals::save_all_kit_settings($document_settings);
        }
        ...
        public static function get_globals() {
            return array(
                'success' => true,
                'typography' => Protuno_Globals::get_typography(),
				'colors' => Protuno_Globals::get_colors(),
                'container_width' => Protuno_Globals::get_elementor_container_breakpoints_width(),
            );
        }
        ...
        private static function build_atomic_ai_snapshot(): string {
            ...
            $atomic = Protuno_Atomic_Globals::get_global_classes_and_variable();
            $out    = array();
        ...
        public static function get_globals_dynamic_css() {
            ...
            if ( self::is_atomic_enabled() && class_exists( 'Protuno_Atomic_Globals' ) ) {
                $atomic     = Protuno_Atomic_Globals::get_global_classes_and_variable();
                $typo_props = array(
                    'font-family', 'font-size', 'font-weight', 'line-height',
                    'letter-spacing', 'text-transform', 'text-decoration', 'font-style',
        ...
        public static function sync_globals( $sync_data ) {
            ...
            if(isset($sync_container_width)){
                Protuno_Globals::set_container_breakpoints_width($sync_container_width);
            }
            ...
            foreach( $sync_color as $color ){
                ...
                if( $action === "DEL" ){
                    Protuno_Globals::delete_global_color( $color_data->id );
                }

                if( $action === "ADD" || $action === "SET" ){
                    Protuno_Globals::set_or_create_color( $color_data->id, $color_data->title, $color_data->value );
                }
            }
            ...
            foreach( $sync_typography as $typography ){              
                ...
                if( $action === "DEL" ){
                    Protuno_Globals::delete_global_typography( $typography_data->id );
                }
                
                if( $action === "ADD" || $action === "SET" ){
                    Protuno_Globals::set_or_create_typography( $typography_data->id, $typography_data->title, $typography_data->value );
                }
            }
