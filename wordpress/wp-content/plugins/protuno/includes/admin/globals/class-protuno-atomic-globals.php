	...
	class Protuno_Atomic_Globals {
        ...
        public static function object_to_array($obj) {
            if (is_object($obj) || is_array($obj)) {
                $ret = (array)$obj;
                foreach ($ret as &$item) {
                    $item = Protuno_Atomic_Globals::object_to_array($item);
                }
                return $ret;
            }
            return $obj;
        ...
        */
        ...
        private static function get_classes_by_prefix(string $prefix, callable $extract): array {
            $global_classes = Protuno_Atomic_Globals::get_global_classes_array();
            $result = [];
        ...
        private static function sync_classes(array $items, string $prefix, callable $build_variants): void {
            ...
            Protuno_Atomic_Globals::save_global_classes_array($global_classes, $old_value);
        }

        /** Normalize sync input: handle single item vs array */
        private static function normalize_sync_input($raw): array {
            $arr = Protuno_Atomic_Globals::object_to_array($raw);
        ...
        public static function get_uich_elementor_variables(): array {
            ...
            foreach ($data['data'] ?? [] as $id => $variable) {
                ...
                $result[] = [
                    ...
                    'value' => Protuno_Atomic_Globals::rgbaToHex($variable['value']['value']),
                ];
            }
            return $result;
        }
        ...
        public static function sync_uich_elementor_variables($sync_data): array {
            if (empty($sync_data->data->color)) return Protuno_Atomic_Globals::get_uich_elementor_variables();

            $kit      = \Elementor\Plugin::$instance->kits_manager->get_active_kit_for_frontend();
            $meta_key = '_elementor_global_variables';
            $vars     = Protuno_Atomic_Globals::object_to_array(json_decode($kit->get_meta($meta_key)));
            ...
            foreach (Protuno_Atomic_Globals::object_to_array($sync_data->data->color) as $variable) {
            ...
            return Protuno_Atomic_Globals::get_uich_elementor_variables();
        }
        ...
        public static function get_elementor_typo_classes(): array {
            return Protuno_Atomic_Globals::get_classes_by_prefix('g-ut', function (array $variant): ?array {
                if (empty($variant['props'])) return null;

                $typo_value = [];
                foreach (Protuno_Atomic_Globals::TYPO_PROPS as $prop) {
        ...
        public static function sync_elementor_typo_classes($sync_data): array {
            if (!empty($sync_data->data->typography)) {
                Protuno_Atomic_Globals::sync_classes(
                    Protuno_Atomic_Globals::normalize_sync_input($sync_data->data->typography),
                    'g-ut',
                    function (array $bp, string $breakpoint): array {
                        $props = [];
                        foreach ($bp as $propName => $propValue) {
                            if (in_array($propName, Protuno_Atomic_Globals::TYPO_SIZE_PROPS)
                        ...
                        return ['meta' => Protuno_Atomic_Globals::variant_meta($breakpoint), 'props' => $props];
                    }
                );
            }

            return Protuno_Atomic_Globals::get_elementor_typo_classes();
        }
        ...
        public static function get_elementor_width_class(): array {
            $global_classes = Protuno_Atomic_Globals::get_global_classes_array();
            $id             = Protuno_Atomic_Globals::get_global_width_class_id();
            $widths         = [];
            ...
            if (empty($widths)) {
                ...
                $auto_margin = Protuno_Atomic_Globals::auto_inline_margin();
                $global_classes['items'][$id] = [
                    'id'       => $id,
                    'type'     => 'class',
                    'label'    => 'elementor-atomic-boxed-width',
                    'variants' => [
                        ['meta' => Protuno_Atomic_Globals::variant_meta('desktop'), 'props' => ['max-width' => Protuno_Atomic_Globals::size(1440),     'margin' => $auto_margin]],
                        ['meta' => Protuno_Atomic_Globals::variant_meta('tablet'),  'props' => ['max-width' => Protuno_Atomic_Globals::size(85, '%'),  'margin' => $auto_margin]],
                        ['meta' => Protuno_Atomic_Globals::variant_meta('mobile'),  'props' => ['max-width' => Protuno_Atomic_Globals::size(90, '%'),  'margin' => $auto_margin]],
                    ],
                ];
                $global_classes['order'][] = $id;
                Protuno_Atomic_Globals::save_global_classes_array($global_classes, $old_value);
        ...
        public static function sync_elementor_width_class($sync_data): array {
            ...
            if (!$has_width_payload) {
                return Protuno_Atomic_Globals::get_elementor_width_class();
            }
            ...
            foreach ($widths as $breakpoint => $value) {
                ...
                $variants[] = [
                    'meta'  => Protuno_Atomic_Globals::variant_meta($breakpoint),
                    'props' => [
                        'max-width' => Protuno_Atomic_Globals::size((float)($m[1] ?? $value), $m[2] ?? 'px'),
                        'margin'    => $auto_margin,
                    ],
                ];
            }
        ...
        public static function get_elementor_padding_classes(): array {
            return Protuno_Atomic_Globals::get_classes_by_prefix('g-up', function (array $variant): ?array {
        ...
        public static function sync_elementor_padding_classes($sync_data): array {
            if (!empty($sync_data->data->padding)) {
                Protuno_Atomic_Globals::sync_classes(
                    Protuno_Atomic_Globals::normalize_sync_input($sync_data->data->padding),
                    'g-up',
                    function (array $bp, string $breakpoint): array {
                        return [
                            'meta'  => Protuno_Atomic_Globals::variant_meta($breakpoint),
                            'props' => [
                                'padding' => [
                                    '$$type' => 'dimensions',
                                    'value'  => [
                                        'block-start'  => ['$$type' => 'size', 'value' => Protuno_Atomic_Globals::parse_size_string($bp['top']    ?? '0px')],
                                        'block-end'    => ['$$type' => 'size', 'value' => Protuno_Atomic_Globals::parse_size_string($bp['bottom'] ?? '0px')],
                                        'inline-start' => ['$$type' => 'size', 'value' => Protuno_Atomic_Globals::parse_size_string($bp['left']   ?? '0px')],
                                        'inline-end'   => ['$$type' => 'size', 'value' => Protuno_Atomic_Globals::parse_size_string($bp['right']  ?? '0px')],
                                    ],
                                ],
                            ],
                        ];
            ...
            return Protuno_Atomic_Globals::get_elementor_padding_classes();
        }
        ...
        public static function get_elementor_border_classes(): array {
            $color_variable_map = [];
            foreach (Protuno_Atomic_Globals::get_uich_elementor_variables() as $variable) {
            ...
            return Protuno_Atomic_Globals::get_classes_by_prefix('g-ub', function (array $variant) use ($color_variable_map): ?array {
        ...
        public static function sync_elementor_border_classes($sync_data): void {
            ...
            Protuno_Atomic_Globals::sync_classes(
                Protuno_Atomic_Globals::normalize_sync_input($sync_data->data->border),
                'g-ub',
                    ...
                    return [
                        ...
                        'props' => [
                            'border-width' => [
                                '$$type' => 'border-width',
                                'value'  => [
                                    'block-start'  => Protuno_Atomic_Globals::size($bp['width']['top']    ?? 0),
                                    'block-end'    => Protuno_Atomic_Globals::size($bp['width']['bottom'] ?? 0),
                                    'inline-start' => Protuno_Atomic_Globals::size($bp['width']['left']   ?? 0),
                                    'inline-end'   => Protuno_Atomic_Globals::size($bp['width']['right']  ?? 0),
                                ],
                            ],
        ...
        public static function get_elementor_border_radius_classes(): array {
            return Protuno_Atomic_Globals::get_classes_by_prefix('g-ur', function (array $variant): ?array {
        ...
        public static function sync_elementor_border_radius_classes($sync_data): void {
            ...
            Protuno_Atomic_Globals::sync_classes(
                Protuno_Atomic_Globals::normalize_sync_input($sync_data->data->borderRadius),
                'g-ur',
                function (array $bp, string $breakpoint): array {
                    return [
                        'meta'  => Protuno_Atomic_Globals::variant_meta($breakpoint),
                        'props' => [
                            'border-radius' => [
                                '$$type' => 'border-radius',
                                'value'  => [
                                    'start-start' => Protuno_Atomic_Globals::size($bp['topLeft']     ?? 0),
                                    'start-end'   => Protuno_Atomic_Globals::size($bp['topRight']    ?? 0),
                                    'end-start'   => Protuno_Atomic_Globals::size($bp['bottomLeft']  ?? 0),
                                    'end-end'     => Protuno_Atomic_Globals::size($bp['bottomRight'] ?? 0),
                                ],
                            ],
                        ],
                    ];
        ...
        public static function get_elementor_shadow_classes(): array {
            return Protuno_Atomic_Globals::get_classes_by_prefix('g-us', function (array $variant): ?array {
        ...
        public static function sync_elementor_shadow_classes($sync_data): void {
            ...
            Protuno_Atomic_Globals::sync_classes(
                Protuno_Atomic_Globals::normalize_sync_input($sync_data->data->shadow),
                'g-us',
                function (array $bp, string $breakpoint): array {
                    $shadowValue = [
                        'hOffset' => Protuno_Atomic_Globals::size($bp['hOffset'] ?? 0),
                        'vOffset' => Protuno_Atomic_Globals::size($bp['vOffset'] ?? 0),
                        'blur'    => Protuno_Atomic_Globals::size($bp['blur']    ?? 0),
                        'spread'  => Protuno_Atomic_Globals::size($bp['spread']  ?? 0),
        ...
        public static function get_elementor_gap_classes(): array {
            return Protuno_Atomic_Globals::get_classes_by_prefix('g-ug', function (array $variant): ?array {
        ...
        public static function sync_elementor_gap_classes($sync_data): void {
            ...
            Protuno_Atomic_Globals::sync_classes(
                Protuno_Atomic_Globals::normalize_sync_input($sync_data->data->gap),
                'g-ug',
                function (array $bp, string $breakpoint): array {
                    return [
                        'meta'  => Protuno_Atomic_Globals::variant_meta($breakpoint),
                        'props' => [
                            'gap' => [
                                ...
                                'value'  => [
                                    'row'    => Protuno_Atomic_Globals::size($bp['row']    ?? 0),
                                    'column' => Protuno_Atomic_Globals::size($bp['column'] ?? 0),
                                ],
                            ],
                        ],
                    ];
        ...
        public static function get_global_classes_and_variable(): array {
            return [
                'width'        => Protuno_Atomic_Globals::get_elementor_width_class(),
                'color'        => Protuno_Atomic_Globals::get_uich_elementor_variables(),
                'typography'   => Protuno_Atomic_Globals::get_elementor_typo_classes(),
                'padding'      => Protuno_Atomic_Globals::get_elementor_padding_classes(),
                'border'       => Protuno_Atomic_Globals::get_elementor_border_classes(),
                'borderRadius' => Protuno_Atomic_Globals::get_elementor_border_radius_classes(),
                'gap'          => Protuno_Atomic_Globals::get_elementor_gap_classes(),
                'shadow'       => Protuno_Atomic_Globals::get_elementor_shadow_classes(),
            ];
        }

        public static function sych_uich_elementor_classes_and_variables_sync($sync_data): array {
            Protuno_Atomic_Globals::sync_elementor_width_class($sync_data);
            Protuno_Atomic_Globals::sync_uich_elementor_variables($sync_data);
            Protuno_Atomic_Globals::sync_elementor_typo_classes($sync_data);
            Protuno_Atomic_Globals::sync_elementor_padding_classes($sync_data);
            Protuno_Atomic_Globals::sync_elementor_border_classes($sync_data);
            Protuno_Atomic_Globals::sync_elementor_border_radius_classes($sync_data);
            Protuno_Atomic_Globals::sync_elementor_gap_classes($sync_data);
            Protuno_Atomic_Globals::sync_elementor_shadow_classes($sync_data);

            return Protuno_Atomic_Globals::get_global_classes_and_variable();
        }
    }
}