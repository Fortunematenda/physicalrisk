...
add_action('wp_enqueue_scripts', function () {
    ...
    $site_key = trim((string) (
        ...
        ?: (defined('TURNSTILE_SITE_KEY') ? TURNSTILE_SITE_KEY : '')
        ?: (string) apply_filters('moss_turnstile_site_key', '')
    ));
    $api_url = trim((string) (
        getenv('MOSS_CONTACT_API_URL')
        ?: (defined('MOSS_CONTACT_API_URL') ? MOSS_CONTACT_API_URL : '')
    )) ?: 'https://moss.physicalrisk.com/api/public/contact';
...
(function () {
  'use strict';
  if (window.__mossContactBooted) return;
  window.__mossContactBooted = true;

  function field(form, name) {
    return form.querySelector('[name="mf-' + name + '"]') || form.querySelector('[name="' + name + '"]');
  }
  function errorNode(input) {
    ...
    if (!node) {
      ...
      node.id = id;
      node.className = 'moss-contact-field-error';
  ...
  function clearMetFormSpinner(form, button) {
    if (button) {
      button.classList.remove('loading', 'is-loading', 'mf-loading', 'disabled');
      button.removeAttribute('disabled');
      button.setAttribute('aria-busy', 'false');
      button.querySelectorAll('.mf-btn-loader, .metform-btn-loader, .attr-spinner, .spinner').forEach(function (n) { n.remove(); });
    }
    if (form) {
      form.classList.remove('loading', 'is-loading', 'mf-loading', 'mf-form-loading');
      form.querySelectorAll('.mf-btn-loader, .metform-btn-loader, .attr-spinner').forEach(function (n) { n.remove(); });
    }
    document.querySelectorAll('.mf-btn-loader, .metform-btn-loader').forEach(function (n) { n.remove(); });
  }

  function initForm(form) {
    if (form.dataset.mossContactReady === '1') return;
    ...
    // Stop MetForm posting to WordPress REST — we own submit → MOSS API.
    ...
    helper.id = helperId;
    helper.className = 'moss-description-helper';
    var counter = document.createElement('span');
    counter.className = 'moss-description-counter';
    var minimum = document.createElement('span');
    minimum.className = 'moss-description-minimum';
    minimum.textContent = 'At least 10 characters required.';
    helper.appendChild(counter);
    helper.appendChild(minimum);
    ...
    if (programme) {
      programme.type = 'text';
      programme.value = 'MOSS Assessment';
    ...
    var captchaHost = document.createElement('div');
    captchaHost.className = 'moss-turnstile';
    if (oldCaptcha) oldCaptcha.replaceWith(captchaHost);
    else {
      var submitBtn = form.querySelector('button[type="submit"], input[type="submit"]');
      if (submitBtn) submitBtn.before(captchaHost);
      else form.appendChild(captchaHost);
    }

    var button = form.querySelector('button[type="submit"], input[type="submit"]');
    var status = document.createElement('div');
    status.className = 'moss-contact-status';
    ...
    function setLoading(loading) {
      ...
      if (loading) {
        ...
        clearMetFormSpinner(form, button);
        if (button.tagName === 'INPUT') button.value = buttonValue || 'Send Message';
        else button.innerHTML = buttonHtml || '<span>Send Message</span>';
        // Keep disabled when Turnstile is unavailable.
        if (!window.MossContact.siteKey) button.disabled = true;
      }
    }
    ...
    function renderCaptcha() {
      if (!window.MossContact.siteKey) {
        setStatus('error', 'The security check is temporarily unavailable. Please refresh or contact support.', false);
    ...
    async function handleSubmit(event) {
      ...
      var controls = {
        fullName: field(form, 'full_name'),
        organisation: field(form, 'organisation'),
        email: field(form, 'email'),
        programmeInterest: programme,
        description: description
      };
      Object.keys(controls).forEach(function (k) { setError(controls[k], ''); });
      setStatus('', '', false);

      var payload = {
        fullName: controls.fullName && controls.fullName.value.trim(),
        organisation: controls.organisation && controls.organisation.value.trim(),
        email: controls.email && controls.email.value.trim(),
        programmeInterest: controls.programmeInterest && controls.programmeInterest.value.trim(),
        description: controls.description && controls.description.value.trim(),
        source: 'wordpress',
      ...
      var invalid = false;
      if (!window.MossContact.siteKey) {
        setStatus('error', 'The security check is temporarily unavailable. Please try again later.', true);
        return;
      }
      ...
      try {
        var response = await fetch(window.MossContact.apiUrl, {
          method: 'POST',
          mode: 'cors',
          credentials: 'omit',
        ...
        if (!response.ok) {
          ...
          var safeMessage = data && typeof data.message === 'string'
            ? data.message
            : 'We could not send your enquiry. Your details have been kept; please try again.';
        ...
        captchaToken = '';
        if (window.turnstile && widgetId !== null) window.turnstile.reset(widgetId);
        form.querySelectorAll('.mf-response-msg, .metform-response-message, .elementor-message').forEach(function (node) {
          node.textContent = '';
          node.hidden = true;
        });
        setStatus('', '', false);
        ...
        setLoading(false);
        clearMetFormSpinner(form, button);
        if (!window.MossContact.siteKey && button) button.disabled = true;
      }
    }
    ...
    if (button) {
      button.addEventListener('click', function (event) {
        // MetForm often starts its spinner on click before submit.
        event.preventDefault();
        event.stopPropagation();
        if (typeof event.stopImmediatePropagation === 'function') event.stopImmediatePropagation();
        clearMetFormSpinner(form, button);
