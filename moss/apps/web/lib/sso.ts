 ...
 * OAuthCallback "state mismatch" — especially right after logout when AuthGate
 * races with signOut. Use a single in-flight promise and a logout guard.
 */
...
async function recordSessionEvent(event: 'APP_LOGOUT' | 'SIGN_OUT') {
  try {
    ...
    await fetch('/api/auth/session-event', {
      ...
      body: JSON.stringify({ event, app: 'moss', email }),
      keepalive: true,
    });
  } catch {
    // ignore
 ...
 * When SSO is on, never fall back to localStorage moss_token — that stale
 * legacy JWT overrides the BFF cookie path and causes Nest 401s.
 */
  ...
  try {
    ...
    if (res.ok) {
      ...
      if (session?.error === 'RefreshTokenError') {
        if (ssoOn) window.localStorage.removeItem('moss_token');
        return null;
      }
