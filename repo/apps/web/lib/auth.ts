...
function assertSsoConfig() {
  ...
  if (missing.length) {
    console.warn(`[repo-sso] Missing env: ${missing.join(', ')}`);
  }
}
...
export const authOptions: AuthOptions = {
  ...
  cookies: {
    sessionToken: {
      name: 'repo.next-auth.session-token',
      options: { httpOnly: true, sameSite: 'lax', path: '/', secure: secureCookies },
    },
    callbackUrl: {
      name: 'repo.next-auth.callback-url',
      options: { sameSite: 'lax', path: '/', secure: secureCookies },
    },
    csrfToken: {
      name: 'repo.next-auth.csrf-token',
      options: { httpOnly: true, sameSite: 'lax', path: '/', secure: secureCookies },
    },
    pkceCodeVerifier: {
      name: 'repo.next-auth.pkce.code_verifier',
      options: { httpOnly: true, sameSite: 'lax', path: '/', secure: secureCookies, maxAge: 900 },
    },
    state: {
      name: 'repo.next-auth.state',
      options: { httpOnly: true, sameSite: 'lax', path: '/', secure: secureCookies, maxAge: 900 },
    },
    nonce: {
      name: 'repo.next-auth.nonce',
      options: { httpOnly: true, sameSite: 'lax', path: '/', secure: secureCookies, maxAge: 900 },
    },
  },
  session: {
  ...
  secret: process.env.NEXTAUTH_SECRET || process.env.AUTH_SECRET || 'repo-dev-secret',
  debug: process.env.SSO_DEBUG === 'true' || process.env.NEXTAUTH_DEBUG === 'true',
  logger: {
    error(code, metadata) {
      const correlationId = `repo-${Date.now().toString(36)}`;
      console.error(`[repo-sso][${correlationId}]`, code, metadata);
    },
  },
};
